...
def landuse(self):
    ...
    extracted_exposure = ex.extract(
                            self.exposure_path, 'multipolygons', ['landuse'])    
...
def buildings(self):
    ...
    """
    extracted_exposure = ex.extract(
                            self.exposure_path, 'multipolygons', ['building'])    
...
def cis(self,infra_type):
    ...
    """
    if infra_type == 'road':
        extracted_exposure = ex.extract(self.exposure_path, 'lines', ['highway','name','maxspeed','lanes','surface'],
...
def _load_flood_data(hazard_map,cell_size,hazard_col,exposure):
    
    unioned_roads = exposure.geometry.buffer(0.01).unary_union.simplify(0.1) # alleen bij echt hoge resolutie

    gridded = create_grid(shapely.box(hazard_map.rio.bounds()[0],hazard_map.rio.bounds()[1],hazard_map.rio.bounds()[2],hazard_map.rio.bounds()[3]),1)
    all_bounds = gpd.GeoDataFrame(gridded,columns=['geometry']).bounds
    
    collect_subsets = []
    for bounds in tqdm(all_bounds.itertuples(),total=len(all_bounds)):
        xdsc = hazard_map[hazard_col].rio.clip_box(
        minx=bounds.minx,
        miny=bounds.miny,
        maxx=bounds.maxx,
        maxy=bounds.maxy,
        ...
        if xdsc.fillna(0).sum() == 0:
            continue
            
        df_subset = xdsc.to_dataframe().dropna(subset=['band_data']).reset_index() 
        
        if len(df_subset) == 0:
            continue
            
        df_subset['geometry'] = gpd.points_from_xy(df_subset.x,df_subset.y)
        df_subset = df_subset.drop(['x','y','band','spatial_ref'],axis=1)
        
        df_subset = gpd.GeoDataFrame(df_subset)
        df_subset = df_subset[df_subset.geometry.intersects(unioned_roads)] # alleen bij echt hoge resolutie
        
        if len(df_subset) == 0:
            continue
        
        df_subset.geometry= shapely.buffer(df_subset.geometry,
                                                  distance=cell_size/2,cap_style='square').values
        df_subset.geometry = _reproject(df_subset,current_crs=f"epsg:4326",approximate_crs = "epsg:3857")  
            
        collect_subsets.append(df_subset)

    return pd.concat(collect_subsets).reset_index(drop=True)
...
def VectorScanner(exposure_file,
    ...
    if isinstance(hazard_file, PurePath):       
        if (hazard_file.parts[-1].endswith('.tif') | hazard_file.parts[-1].endswith('.tiff')): 
            ...
            # load hazard data in steps to prevent out of memory issues 
            hazard = _load_flood_data(hazard_map,cell_size,hazard_col,exposure)
    ...
    exposure.geometry = _reproject(exposure,current_crs=f"epsg:4326",approximate_crs = "epsg:3857")  
    ...
    for curve_id in curves:
        ...
        hazard_intensity = np.float64(hazard_intensity)*100
        fragility_values = np.float64(fragility_values)

        # estimate damage per curve  
        collect_damage = {}
        for asset in tqdm(overlay_assets.groupby('asset'),total=len(overlay_assets.asset.unique())): #group asset items for different hazard points per asset and get total number of unique assets
            ...
            collect_damage[asset[0]] = _get_damage_per_element(asset,hazard_numpified,element_geom,hazard_intensity,
                                                                                fragility_values,maxdam_element)

        asset_damage = pd.Series(collect_damage)
        asset_damage.columns = [curve_id]
        collect_curve_damages.append(asset_damage)
        
    all_damages = pd.concat(collect_curve_damages,axis=1)
    all_damages.columns = curves.columns
    damaged_assets = exposure.merge(all_damages,left_index=True,right_index=True,how='outer')
    damaged_assets = damaged_assets.drop(['buffered'],axis=1)
    damaged_assets.crs = 3857
    damaged_assets = damaged_assets.to_crs(4326)
    damaged_assets.damage = damaged_assets[curves.columns].fillna(0)
