...
def load_flood_as_dataframe(hazard_file,hazard_type='pluvial'):
    """[summary]
...
def regional_analysis(country,reg_index,asset_type='roads',hazard='pluvial'):
    ...
    try:
        ...
        for return_period in tqdm(return_periods,total=len(return_periods),desc='Assessment for {} {} {} {}'.format(country,reg_index,hazard,asset_type)):
            ...
            hazard_reg = df_ds.iloc[sindex_hazard.query(lev1_gadm.iloc[reg_index].geometry,predicate='intersects')].reset_index()
...
if __name__ == "__main__":
    ...
    reg_indices = np.arange(0,82,1)

    reg_index = 47
    regional_analysis(reg_index,asset_type='roads',hazard='wildfires')
    #with Pool(20) as pool: 
    #    pool.map(regional_analysis, reg_indices)

    #with Pool(20) as pool: 
    #     pool.map(partial(regional_analysis, asset_type='roads', hazard='pluvial'), reg_indices,chunksize=1)
