...
def merge_energy_datatypes1(osm_path):
    """
    Function to extract and merge energy assets with different datatypes from OpenStreetMap  
    Arguments:
        *osm_path* : file path to the .osm.pbf file of the region 
    ...
    #extract polygon data
    df_poly = power_polygon1(osm_path) #extract required data
    df_poly['geometry'] =pygeos.buffer(df_poly.geometry,0) #avoid intersection
...
def power_polygon1(osm_path):
    """
    Function to extract energy polygons from OpenStreetMap, adjusted string conditions  
    Arguments:
        *osm_path* : file path to the .osm.pbf file of the region 
        for which we want to do the analysis.        
    Returns:
    ...
    df['asset'].loc[df['asset'].str.contains('substation', case=False)]  = 'substation' #specify row
    df['asset'].loc[df['asset'].str.contains('plant', case=False)] = 'plant' #specify row
...
def airports_polygon(osm_path):
    ...
    Returns:
        *GeoDataFrame* : a geopandas GeoDataFrame with all unique airport multipolygons.   
    """ 
    return (retrieve(osm_path,'multipolygons',['aeroway'])).rename(columns={'aeroway': 'asset'}) 

def airports_line(osm_path):
    """
    Function to extract airport multipolygons from OpenStreetMap    
    Arguments:
        *osm_path* : file path to the .osm.pbf file of the region 
    ...
    Returns:
        *GeoDataFrame* : a geopandas GeoDataFrame with all unique airport multipolygons.   
    """ 
    df = retrieve(osm_path,'lines',['other_tags']).rename(columns={'aeroway': 'asset'})
    
    df = df.loc[(df.other_tags.str.contains('aeroway'))]  #keep rows containing power data
    df = df.reset_index(drop=True).rename(columns={'other_tags': 'asset'})   

    df['asset'].loc[df['asset'].str.contains('"aeroway"=>"runway"', case=False)] = 'runway' #specify row
    df = df.loc[(df.asset == 'runway')]
    
    return df.reset_index(drop=True) 

def merge_airport_datatypes(osm_path):
    """
    Function to combine extract airport multipolygons and lines from OpenStreetMap    
    Arguments:
        *osm_path* : file path to the .osm.pbf file of the region 
        for which we want to do the analysis.        
    Returns:
    ...
    """ 
    #extract polygon data
    df_polygon = airports_polygon(osm_path) #extract required data
    df_polygon['geometry'] =pygeos.buffer(df_polygon.geometry,0) #avoid intersection
    
    #extract line data
    df_line = airports_line(osm_path) #extract required data

    return pandas.concat([df_line, df_polygon], ignore_index=True) 
